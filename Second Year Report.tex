...
We will pick policy, $\pi^{\text{R}}$ over $\pi^{\text{NR}}$
\begin{align*}
...
No we will work backwards, aiming to use backwards recursion to work through to some multi-stage threshold policy, $\pi^{\text{TH}}(\bm{v}_{\text{crit}})$ where $(\bm{v}_{\text{crit}})_{i}$ is the threshold on the state space row of $s=i$. We expect this threshold to be triangular on the state space, that is that $\bm{v}_{\text{crit}}$ is a non-decreasing vector.

\begin{definition}[Multi-stage threshold policy]

\end{definition}

As an example we will now start to work out what to  do in states $s=B$ and how to work out the threshold. Well the same process as in Section \ref{Section:Single node solution} holds and we will renew if $g \leq c (\lambda R + v)$ otherwise we will wait and renew later. 
...
We could split ourself into cases, of $v$ such that $g \leq c (\lambda R +v)$ (so if we wait we renew in $s=B$) or otherwise we wait till $s=B+1$.
