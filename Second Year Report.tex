...
This change does not affect the theory that much, the problem can still be reduced to the single node version of the instantaneous moving problem. However there is a slight change here, which results in a slight change to the optimal solution and hence the index which is suggested.
...
For now we will assume that the value of the game for all tree graphs has been found. We now consider the role of spanning tree graph, $T=(N_{T},E_{T})$, in a general graph, $G=(N,E)$. Then we know $V(G) \leq V(T)$, as the introduction of more edges cannot harm the patroller.
...
The patroller's strategy is some walk (with possible waiting), we will assume as before that they walk along the edges at unit speed. The decisions are which edge to traverse next (which is equivalent to which node to walk to).
