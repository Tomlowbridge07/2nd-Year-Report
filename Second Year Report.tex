...
c_{i} \lambda_{i} \left( \int_{s_{j}-1}^{s_{j}} P(X_{j} \leq t) dt - \int_{0}^{1} P(X_{j} \leq t) dt \right)  \text{ if } j=i \\
c_{i} \lambda_{i} \int_{s_{j}-1}^{s_{j}} P(X_{j} \leq t) dt \text{ if } j \neq i \\
\end{cases}
\end{align*}
...
The patroller incurs costs for all attackers able to complete their attacks.

\subsection{Problem relaxation}
\subsection{Discrete attack time assumption}
